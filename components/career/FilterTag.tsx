...
const FilterTag = ({ type, value, onRemove }: FilterTagProps) => {
  ...
  return (
    <div
      className={`flex items-center px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm ${tagStyle}`}
    >
      ...
      <button
        onClick={onRemove}
        className={`ml-1 sm:ml-2 rounded-full p-0.5 transition-colors cursor-pointer bg-transparent ${hoverBgColor}`}
      >
        <svg
